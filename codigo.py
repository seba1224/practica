...
#reemplazai tu x,y que quieres fittiar y listoco
datos_composite = np.sum(spec, axis=1)
x = vel[49:113]
y = datos_composite
...
p_optimo_gauss, a_covarianza_gauss = scipy.optimize.curve_fit(Modelo_gauss,
                                                              x, y[:, 0], p0)
desv_fit = p_optimo_gauss[4]*u.km/u.s

plt.clf()
plt.plot(x, Modelo_gauss(x, *p_optimo_gauss), color='r', label='fitting')
plt.plot(x, y, label='medicion')
plt.xlabel('vel[km/s]')
plt.ylabel('Composite spectrum[K]')
plt.title('Grafico de espectro compuesto y su ajuste')
plt.legend()
plt.show()

R_eq = np.sqrt(spec.shape[1]*area/np.pi).to(u.pc)
...
masa_virial0 = virial_mass(desv_fit, 0, R_eq)
masa_virial1 = virial_mass(desv_fit, 1, R_eq)
masa_virial2 = virial_mass(desv_fit, 2, R_eq)
