...
ax3.plot(integ_thin_flat[index_sort], integ_wide_flat[index_sort], 'bo',
         label='datos medidos')
ax3.set_title('Integrated Temperature')
ax3.set_xlabel('thin')
ax3.set_ylabel('wide')

A = np.vstack([integ_thin_flat[index_sort], np.ones(len(integ_thin_flat))]).T
reg = np.linalg.lstsq(A, integ_wide_flat[index_sort])
ax3.plot(integ_thin_flat[index_sort], integ_thin_flat[index_sort]*reg[0][0] +
...
ax6.plot(peak_thin_flat[index_sort2], peak_wide_flat[index_sort2], 'bo',
         label='datos medidos')
ax6.set_title('Peak Temperature')
ax6.set_xlabel('thin')
ax6.set_ylabel('wide')

A2 = np.vstack([peak_thin_flat[index_sort2], np.ones(len(peak_thin_flat))]).T
reg2 = np.linalg.lstsq(A2, peak_wide_flat[index_sort2])
ax6.plot(peak_thin_flat[index_sort2], peak_thin_flat[index_sort2]*reg2[0][0] +
